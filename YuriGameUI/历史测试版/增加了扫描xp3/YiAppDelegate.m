...
static NSArray<NSString *> *ygXp3PriorityNames(void) {
...
static NSString *ygCaseInsensitiveMatch(NSString *fileName, NSArray<NSString *> *candidates) {
    NSString *lowerFile = fileName.lowercaseString;
    for (NSString *candidate in candidates) {
        if ([lowerFile isEqualToString:candidate.lowercaseString]) {
            return fileName;
        }
    }
    return nil;
...
static NSString *ygSelectXp3FileInDirectory(NSString *directoryPath) {
    NSFileManager *fm = [NSFileManager defaultManager];
    NSError *error = nil;
    NSArray<NSString *> *files = [fm contentsOfDirectoryAtPath:directoryPath error:&error];
    if (error || files.count == 0) {
        return nil;
    }
    for (NSString *priorityName in ygXp3PriorityNames()) {
        for (NSString *file in files) {
            NSString *matched = ygCaseInsensitiveMatch(file, @[priorityName]);
            if (matched) {
                return matched;
            }
        }
...
static void ygCollectGameDirectoriesFromInboxItem(NSString *inboxItemPath, NSMutableArray<NSString *> *results) {
    NSFileManager *fm = [NSFileManager defaultManager];
    BOOL isDir = NO;
    if (![fm fileExistsAtPath:inboxItemPath isDirectory:&isDir] || !isDir) {
        return;
    }
    if (ygSelectXp3FileInDirectory(inboxItemPath)) {
        [results addObject:inboxItemPath];
        return;
    }
    NSError *error = nil;
    NSArray<NSString *> *items = [fm contentsOfDirectoryAtPath:inboxItemPath error:&error];
    if (error) {
        return;
    }
    for (NSString *item in items) {
        NSString *childPath = [inboxItemPath stringByAppendingPathComponent:item];
        BOOL childIsDir = NO;
        if ([fm fileExistsAtPath:childPath isDirectory:&childIsDir] && childIsDir && ygSelectXp3FileInDirectory(childPath)) {
            [results addObject:childPath];
        }
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"游戏扫描" message:@"是否需要扫描新导入的游戏" preferredStyle:UIAlertControllerStyleAlert];
        ...
        [alert addAction:[UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            [self performGameScan];
        }]];
        [self presentViewController:alert animated:YES completion:nil];
    }
}

- (void)performGameScan {
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documents = paths.firstObject;
    NSString *inboxPath = [documents stringByAppendingPathComponent:@"Inbox"];
    NSString *xp3PlayerPath = [documents stringByAppendingPathComponent:@"XP3Player"];
    NSString *gameConfigPath = [documents stringByAppendingPathComponent:@"Game"];
    NSFileManager *fm = [NSFileManager defaultManager];
    [fm createDirectoryAtPath:xp3PlayerPath withIntermediateDirectories:YES attributes:nil error:nil];
    [fm createDirectoryAtPath:gameConfigPath withIntermediateDirectories:YES attributes:nil error:nil];
    BOOL inboxIsDir = NO;
    if (![fm fileExistsAtPath:inboxPath isDirectory:&inboxIsDir] || !inboxIsDir) {
        return;
    }
    NSMutableArray<NSString *> *candidates = [NSMutableArray array];
    NSError *error = nil;
    NSArray<NSString *> *inboxItems = [fm contentsOfDirectoryAtPath:inboxPath error:&error];
    if (error) {
        return;
    }
    for (NSString *item in inboxItems) {
        NSString *fullPath = [inboxPath stringByAppendingPathComponent:item];
        ygCollectGameDirectoriesFromInboxItem(fullPath, candidates);
    }
    for (NSString *gameDirectory in candidates) {
        NSString *selectedXp3 = ygSelectXp3FileInDirectory(gameDirectory);
        if (!selectedXp3) {
            continue;
        }
        NSString *gameName = gameDirectory.lastPathComponent;
        NSString *destinationPath = [xp3PlayerPath stringByAppendingPathComponent:gameName];
        if ([fm fileExistsAtPath:destinationPath]) {
            [fm removeItemAtPath:destinationPath error:nil];
        }
        if (![fm moveItemAtPath:gameDirectory toPath:destinationPath error:nil]) {
            continue;
        }
        NSString *xp3FullPath = [[destinationPath stringByAppendingPathComponent:selectedXp3] stringByStandardizingPath];
        NSString *configPath = [gameConfigPath stringByAppendingPathComponent:[gameName stringByAppendingPathExtension:@"txt"]];
        NSString *configContent = [NSString stringWithFormat:@"1:XP3Player.appex\n2:%@", xp3FullPath];
        [configContent writeToFile:configPath atomically:YES encoding:NSUTF8StringEncoding error:nil];
    }
}
